...
pub fn lambda1_deps<A,B,FN:IsLambda1<A,B>>(f: &FN) -> Vec<GcNode> {
    f.deps_op().map(|deps| deps.clone()).unwrap_or_else(|| Vec::new())
}

pub fn lambda2_deps<A,B,C,FN:IsLambda2<A,B,C>>(f: &FN) -> Vec<GcNode> {
    f.deps_op().map(|deps| deps.clone()).unwrap_or_else(|| Vec::new())
}

pub fn lambda3_deps<A,B,C,D,FN:IsLambda3<A,B,C,D>>(f: &FN) -> Vec<GcNode> {
    f.deps_op().map(|deps| deps.clone()).unwrap_or_else(|| Vec::new())
}

pub fn lambda4_deps<A,B,C,D,E,FN:IsLambda4<A,B,C,D,E>>(f: &FN) -> Vec<GcNode> {
    f.deps_op().map(|deps| deps.clone()).unwrap_or_else(|| Vec::new())
}

pub fn lambda5_deps<A,B,C,D,E,F,FN:IsLambda5<A,B,C,D,E,F>>(f: &FN) -> Vec<GcNode> {
    f.deps_op().map(|deps| deps.clone()).unwrap_or_else(|| Vec::new())
}

pub fn lambda6_deps<A,B,C,D,E,F,G,FN:IsLambda6<A,B,C,D,E,F,G>>(f: &FN) -> Vec<GcNode> {
    f.deps_op().map(|deps| deps.clone()).unwrap_or_else(|| Vec::new())
}

pub trait IsLambda1<A,B> {
    fn call(&mut self, a: &A) -> B;
    fn deps_op<'r>(&'r self) -> Option<&'r Vec<GcNode>>;
}
...
pub fn lambda1<A,B,FN:FnMut(&A)->B>(f: FN, deps: Vec<GcNode>) -> Lambda<FN> {
    Lambda { f, deps }
}

pub fn lambda2<A,B,C,FN:FnMut(&A,&B)->C>(f: FN, deps: Vec<GcNode>) -> Lambda<FN> {
    Lambda { f, deps }
}

pub fn lambda3<A,B,C,D,FN:FnMut(&A,&B,&C)->D>(f: FN, deps: Vec<GcNode>) -> Lambda<FN> {
    Lambda { f, deps }
}

pub fn lambda4<A,B,C,D,E,FN:FnMut(&A,&B,&C,&D)->E>(f: FN, deps: Vec<GcNode>) -> Lambda<FN> {
    Lambda { f, deps }
}

pub fn lambda5<A,B,C,D,E,F,FN:FnMut(&A,&B,&C,&D,&E)->F>(f: FN, deps: Vec<GcNode>) -> Lambda<FN> {
    Lambda { f, deps }
}

pub fn lambda6<A,B,C,D,E,F,G,FN:FnMut(&A,&B,&C,&D,&E,&F)->G>(f: FN, deps: Vec<GcNode>) -> Lambda<FN> {
    Lambda { f, deps }
}
